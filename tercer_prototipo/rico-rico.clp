...
(defmessage-handler MAIN::PlatoAbstracto calcula-puntuacion-cena "" (?comida)
	(bind ?puntos 0)
	(bind ?descripcion "N/A")
	(if (or (and (not ?comida) (send ?self:Plato get-AptoCena)) (and ?comida (not (send ?self:Plato get-AptoCena))))
	    then 
	        (bind ?puntos 20)
	        (bind ?descripcion "Plato apto para la cena o comida que se va celebrar")
    )
...
(deffunction generar-platos-abstractos-ordenados ()
    (bind ?listaPlatosOrdenados create$)
	(bind ?listaPlatosAbstractos (find-all-instances ((?inst PlatoAbstracto)) TRUE))
	(loop-for-count (?i 1 (length$ ?listaPlatosAbstractos)) do
		(bind ?platoAbstracto (nth$ ?i ?listaPlatosAbstractos))
		(bind ?min ?i)
        (loop-for-count (?j ?i (length$ ?listaPlatosAbstractos)) do
            (if (> (send (nth$ ?j ?listaPlatosAbstractos) get-Puntuacion) 
                   (send ?platoAbstracto get-Puntuacion))
               then (bind ?min ?j)
            )
        )
        (insert$ ?listaPlatosOrdenados (+ 1 (length$ ?listaPlatosOrdenados)) (nth$ ?min ?listaPlatosAbstractos)) 
    )
    
    (loop-for-count (?i 1 (length$ ?listaPlatosOrdenados)) do
        (send (nth$ ?i ?listaPlatosOrdenados) imprimir-debug)
    )
)
...
)(length$ ?listaPlatosAbstractos)) do
...
(defrule solucionAbstracta::calcular-puntuaciones ""
	...
	(loop-for-count (?i 1 (length$ ?listaPlatosAbstractos)) do
		...
		(send ?platoAbstracto calcula-puntuacion-estilo ?estilo)
 		(send ?platoAbstracto calcula-puntuacion-cena ?comida)
...
(defrule solucionConcreta::ordenar-platos-abstractos ""
    (not (imprimir))
    =>
    (generar-platos-abstractos-ordenados)
    (assert (imprimir))
)
